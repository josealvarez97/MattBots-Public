...
So far, I have done most of the programming, and **every single sample script or excerpt I have showcased here is part of what I have done by myself**. [@oswilehi](https://github.com/oswilehi), *a good old friend and great teammate*, has developed the back-end REST API in Node.js, that lives on a server powered by GCloud App Engine, that handles the communication between the Unity Mobile Application and a MySQL database through HTTP requests (that I do from the game with the Unity Web Requests library).
...
***The public BETA release is just out, and although we are not done yet, it's been a wonderful adventure so far alongside this great team.***
